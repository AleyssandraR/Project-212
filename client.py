...
def sendMessage():
    ...
    if(msgtosend=='y' or msgtosend=='Y'):
        textarea.insert(END, '\n'+'Please wait for this file to download...')
        textarea.see('end')
        HOSTNAME='127.0.0.1'
        USERNAME='lftpd'
        PASSWORD='lftpd'
        ...
        FTPServer.getcwd('share_files')
        fName=filetodownload
        localfilename=os.path.join(download_path, fName)
        file=open(localfilename, 'wb')
        FTPServer.retrbinary('RETR'+fName, file.write)
        FTPServer.dir()
        file.close()
        FTPServer.quit()
        print(f'File successfully downloaded to the path {download_path}')
        textarea.insert(END, '\n'+'File successfully downloaded to the path'+download_path)
        textarea.see('end')
